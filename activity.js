...
const addentry = (a) => a.push({title: "guy", author: "dude"})
addentry(bookList)
console.log(bookList)

const removentry = (x, y) => x.filter(function(element){
    return element.title !== y
});

let a = removentry(bookList, "guy")
console.log(a)

const listout = (a, x) => ((x==="title") ? a.forEach(element => {console.log(element.title)}) : a.forEach(element => {console.log(element.author)}));

listout(bookList, "title")
...
const isneg = (x) => ((x>0) ? "positive" : "negative")
console.log(isneg(1))
